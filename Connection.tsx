...
/** Returns true if the tariff name marks a router-only subscription. */
function isRouterTariff(tariffName: string | null | undefined): boolean {
  if (!tariffName) return false;
  return /router|роутер|openwrt|podkop/i.test(tariffName);
}
...
export default function Connection() {
  ...
  // ── Router tariff detection ───────────────────────────────────────────────
  // Uses the same queryKey as Subscription.tsx — free from cache if user came
  // from the subscription page, one extra request otherwise.
  ...
  const routerTariff = isRouterTariff(
    subscriptionStatus?.subscription?.tariff_name,
  );
  // ─────────────────────────────────────────────────────────────────────────
  ...
  // ── Router tariff: show Podkop guide instead of standard installation ─────
  if (routerTariff) {
    return (
      <PodkopGuide
