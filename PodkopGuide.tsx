...
export default function PodkopGuide({ subscriptionUrl, onGoBack, isTelegramWebApp }: Props) {
      ...
      .then((raw) => {
        const parseLines = (text: string) =>
          text
            .split('\n')
            .map((l) => l.trim())
            .filter((l) => l.startsWith('vless://') || l.startsWith('vmess://') || l.startsWith('ss://'));
        try {
          const decoded = atob(raw.trim());
          const keys = parseLines(decoded);
          setVlessKeys(keys.length ? keys : parseLines(raw));
        } catch {
          setVlessKeys(parseLines(raw));
        }
      })
      .catch(() => setVlessKeys([]))
  ...
  return (
    <div className="space-y-5 pb-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="rounded-lg bg-accent-500/10 px-2.5 py-1 text-xs font-medium text-accent-400 ring-1 ring-accent-500/20">
          OpenWRT
        </span>
      </div>
      ...
      <div className="rounded-2xl border border-dark-700/60 bg-dark-800/60 p-4">
        <p className="mb-3 text-xs font-medium uppercase tracking-wider text-dark-400">
          Ваши VLESS-ключи
        </p>
        ...
        {!loading && vlessKeys.length === 0 && (
          <p className="text-xs italic text-dark-500">Ключи недоступны</p>
        )}
              ...
              return (
                <div key={i} className="rounded-xl border border-dark-700/50 bg-dark-900/60 p-3">
                  <div className="mb-2 flex items-center justify-between gap-2">
                    <span className="truncate text-xs font-medium text-dark-300">{name}</span>
                    <button
                      ...
                      className={`flex shrink-0 items-center gap-1.5 rounded-lg px-2.5 py-1 text-xs font-medium transition-all ${
                        isCopied
                          ...
                          : 'bg-accent-500/10 text-accent-400 ring-1 ring-accent-500/20 hover:bg-accent-500/20'
                      }`}
                    >
      ...
      <div className="rounded-2xl border border-dark-700/60 bg-dark-800/40 p-4">
        <p className="mb-4 text-xs font-medium uppercase tracking-wider text-dark-400">
          Инструкция по настройке Podkop
        </p>
        <div className="flex flex-col gap-4">
          ...
          <Step num={4} title="Выберите режим работы">
            Рекомендуем <b className="text-dark-200">Proxy only blocked</b> — трафик на
            заблокированные сайты пойдёт через VPN, остальной — напрямую.
          </Step>
          <div className="h-px bg-dark-700/40" />
